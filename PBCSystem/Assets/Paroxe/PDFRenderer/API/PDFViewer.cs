...
{
    /// <summary>
    /// pdfviewer是一个统一的ui组件，允许您可视化pdf文档
    /// </summary>
    public class PDFViewer : UIBehaviour, IPDFDevice, IPDFColoredRectListProvider
    {
        ...
        [SerializeField] private Color m_PageColor = new Color(1f,1f,1f,0.8f);
            ...
            set
            {
                if (m_ShowTopBar != value)
                {
                    ...
                    if (!m_ShowTopBar)
                    {
                        ...
                        m_Internal.m_Viewport.offsetMax = new Vector2(m_Internal.m_Viewport.offsetMax.x, 0f);
        ...
        private void AdjustCurrentSearchResultDisplayed()
        {
            if (m_SearchResults != null && m_SearchResults.Length > 0)
            {
                if (!m_CurrentPageRange.ContainsPage(m_CurrentSearchResult.PageIndex))
                {
                    ...
                    Debug.Log(disMinPage);
                    Debug.Log(disMaxPage);
                    int nearestPage = -1;
