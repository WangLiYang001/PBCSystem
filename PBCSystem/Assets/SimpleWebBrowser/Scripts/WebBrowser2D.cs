...
namespace SimpleWebBrowser
{
        ...
        IPointerUpHandler
    {
        ...
        public void InitEngineNew(int width, int height, string sharedfilename, int port, string initialURL, bool enableWebRTC)
        {
        ...
        {
            Debug.Log("SendMouseButtonEvent:" + btn + "-"+ type);
            MouseMessage msg = new MouseMessage
            {
                Type = type,
                X = x,
        ...
        public void DragBrowser(bool first,bool end)
        {
            var _raycaster = GetComponentInParent<GraphicRaycaster>();
            var _input = FindObjectOfType<StandaloneInputModule>();
        ...
        public void GetOffset(int pz)
        {
            int px = 0;
            int py = 0;
            BrowserScroll(px, py, pz);
        }
        public void BrowserScroll(int px, int py, int pz)
        {
            if (pz != 0)
            {
                MouseMessage msg = new MouseMessage
